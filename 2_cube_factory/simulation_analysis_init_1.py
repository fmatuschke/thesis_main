...
parameters = list(itertools.product(df.omega.unique(), df.psi.unique(), ["init", "solved"]))
random.shuffle(parameters)
parameters = parameters[comm.Get_rank()::comm.Get_size()]

with tqdm(total=len(parameters) * len(f0_incs) * 2) as pbar:
        for omega, psi, state in parameters:
            if os.path.isfile(
