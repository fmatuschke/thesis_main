...
def run(input):
    ...
    solver.omp_num_threads = args.num_threads
    ...
    for i in tqdm(range(1, args.max_steps), leave=False):
        if solver.step():
            break
        ...
        if i > args.max_steps / 2 and overlap <= 0.001:
            logger.info("i > args.max_steps / 2 and overlap <= 0.001")
            break
...
if __name__ == "__main__":
    with mp.Pool(processes=args.num_proc) as pool:
        for i in tqdm(pool.imap_unordered(run, PARAMETER),
                      total=len(PARAMETER),
                      smoothing=0):
            pass
