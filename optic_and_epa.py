...
if not ".pkl" in args.output and not ".csv" in args.output:
    raise ValueError("output not recognized: " + args.output)
...
df = pd.DataFrame()
voxel_size_ref = str(min([float(i) for i in h5py.File(INPUT_FILES[0], 'r')]))
