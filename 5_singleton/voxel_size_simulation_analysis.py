...
if __name__ == "__main__":
    ...
    for f0_inc in sorted(df.f0_inc.unique()):
        for vs in sorted(df.voxel_size.unique()):
            ...
            if np.any(np.isnan(phi)) or np.any(np.isnan(phi_inc)):
                print(phi, phi_inc)
            h, x = np.histogram(phi, hist_phi(4 * 18), density=True)
            if np.any(np.isnan(h)) or np.any(np.isnan(x)):
                print("1:", h, x)
            x = x[:-1] + (x[1] - x[0]) / 2
            h = h / np.amax(h)
