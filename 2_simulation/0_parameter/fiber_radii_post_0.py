...
def run_orientation(file):
    with h5py.File(file, 'r') as h5f:
        ...
        voxel_size = h5f['/'].attrs['voxel_size']
        v0 = h5f['/'].attrs['v0']
    ...
    return pd.DataFrame([[
        radius, voxel_size, v0, omega, psi, f0_inc, f1_rot, pixel_size, phi,
        theta
    ]],
                        columns=[
                            "radius", "voxel_size", "v0", "omega", "psi",
                            "f0_inc", "f1_rot", "pixel_size", "f_phi", "f_theta"
                        ])
...
def run(file):
    ...
    with h5py.File(file, 'r') as h5f:
        ...
        for setup in h5f[f'simpli']:
            for species in h5f[f'simpli/{setup}']:
                for model in h5f[f'simpli/{setup}/{species}']:
                    h5f_sub = h5f[f'simpli/{setup}/{species}/{model}']
...
df.to_pickle(os.path.join(args.input, "fiber_radii_orientations.pkl"))
...
df = pd.concat(df, ignore_index=True)
df.to_pickle(os.path.join(args.input, "fiber_radii_post_0.pkl"))
