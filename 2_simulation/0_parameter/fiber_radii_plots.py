...
df = pd.read_pickle(
    os.path.join(os.path.join(args.input, "fiber_radii_post_1.pkl")))
df = df[df.f1_rot == 0]
...
for _, p in tqdm.tqdm(parameters):
    ...
    for m in [0, 1]:
        ...
        for r in df_.radius.unique():
            df__ = df_[(df_.radius == r)].copy()
            df__ = df__.apply(pd.Series.explode).reset_index(drop=True).apply(
            ...
            df__.loc[df__.epa_ret_diff_rel == 0, "epa_ret_diff_rel"] = np.nan
            df__.loc[df__.data_diff == 0, "data_diff"] = np.nan
            df__.drop(["radius", "setup", "species", "model"],
                      inplace=True,
                      axis=1)
