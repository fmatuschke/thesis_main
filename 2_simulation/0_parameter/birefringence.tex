...
\end{axis}
%
\node[anchor=north] at (PC |- current bounding box.south) {\ref{legendT\k\l}};
% 
% CENTER AXIS
...
\end{axis}
%
\node[anchor=north] at (PC |- current bounding box.south) {\ref{legendR\k\l}};
% 
% CENTER AXIS
\coordinate (P) at (current bounding box.west);
\path[] let \p1 = ($(PC)-(P)$) in (P) -- ($(P) + (2*\x1,0)$);
