    ...
    for m in [0, 1]:
        ...
        for vs in tqdm.tqdm(df_.voxel_size.unique(), leave=False):
            for r in df_.radius.unique():
                ...
                df__.drop(["voxel_size", "radius", "setup", "model"],
                          inplace=True,
                          axis=1)
                ...
                df__.to_csv(
                    os.path.join(
                        args.input, "results",
                        f"vs_stats_omega_{omega}_psi_{psi}_f0_inc_{f0_inc}_mode_{model}_setup_{setup}_vs_{vs}_r_{r}_m_{m}.csv"
                    ),
                    index=False,
                    # float_format='%.9f',
                    na_rep="nan")
