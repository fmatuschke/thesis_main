...
\foreach \o[count=\oi from 0] in {0,30,60,90}{
   ...
   \end{scope}
   % 
   \node[anchor=south, yshift=1.3cm] at ($(A\oi)!0.5!(B\oi)$) {\footnotesize $\Omega = \SI{\o}{\degree}$};
}
...
\draw[dashed] (current bounding box.west) -- (current bounding box.east);
%
\node[anchor=south] at (current bounding box.north) {$\Psi = \SI{\p0}{\percent}$};
%
...
%
\node[anchor=south] at (current bounding box.north) {$\Psi = \SI{100}{\percent}$};
%
\begin{scope}[shift={(current bounding box.south)}, xshift=-5cm, yshift=0em]
\begin{axis}[
    hide axis,
