\documentclass[tikz]{standalone}
\usepackage{float}              % [H]
\usepackage{graphicx}           % (pdf, png, jpg, eps)
\usepackage{pgfplots}
\usepackage{siunitx}
\usepackage{currfile}
\usepackage{ifthen}
\usepackage{relsize}
\usepackage{tikz}
\usetikzlibrary{calc}
\usepackage{pgfplots}
\usepackage{pgfplotstable}
\pgfplotsset{compat=1.17}
\usepgfplotslibrary{colorbrewer}
\usepgfplotslibrary{polar}
\usepgfplotslibrary{groupplots}
\usepgfplotslibrary{statistics}
\tikzset{>=latex}
% 
\newlength{\width}
\newlength{\height}
\setlength{\width}{13.87303cm} %5.462 inch
\setlength{\height}{22.37076cm} %8,807 inch
\def\datapath{../../__PATH__/hist}
\directlua0{
  pdf.setinfo ("/Path: (__PATH__)")
}
% 
\input{parameters}
% 
\definecolor{dark21}{HTML}{1b9e77}
\definecolor{dark22}{HTML}{d95f02}
\definecolor{dark23}{HTML}{7570b3}
\colorlet{RED}{dark22}
\colorlet{GREEN}{dark21}
\colorlet{BLUE}{dark23} 
% 
\begin{document}
% 
\pgfplotsset{%
  colormap/cividis/.style={colormap={cividis}{[1pt]
    rgb(0pt)=(0.00000000, 0.13511200, 0.30475100);
    rgb(25pt)=(0.25974000, 0.30512000, 0.42281000);
    rgb(50pt)=(0.48514100, 0.48245100, 0.47038400);
    rgb(75pt)=(0.73242200, 0.67736400, 0.42571700);
    rgb(100pt)=(0.99573700, 0.90934400, 0.21777200);
}}}
% 
\pgfplotsset{%
   PStyle/.style={%
      every axis/.append style={font=\tiny},
      width = 0.17\width, height = 0.17\width,
      scale only axis,
      xtick={0,45,90,135,180,225,270,315},
      xticklabels={$\SI{0}{\degree}$,$\SI{45}{\degree}$,$\SI{90}{\degree}$,
                  $\SI{135}{\degree}$,$\SI{180}{\degree}$,$\SI{225}{\degree}$,
                  $\SI{270}{\degree}$,$\SI{315}{\degree}$},
      ytick={60,30},
      yticklabels={$\SI{30}{\degree}$,$\SI{60}{\degree}$},
      yticklabel style=white,
      tickwidth=0, xtick distance = 45,
      separate axis lines, y axis line style= { draw opacity=0 },
      ymin=0, ymax=90,
      axis on top=true,
      colormap/cividis,
      point meta min=0,
      % point meta max=0.693147181, %ln(2)
      % colorbar, colorbar style={ytick=\empty, width=5pt}, 
   }
}
% 
\pgfplotsset{%
   CBStyle/.style={%
      hide axis,
      scale only axis,
      height=0pt, width=0pt,
      xmin=0, xmax=1, ymin=0, ymax=1,
      colormap/cividis,
      colorbar horizontal,
      point meta min = 0,
      point meta max = 1,
      colorbar style={
         width=10cm,
         height=0.42cm,
         xticklabel style={tick label style={font=\footnotesize}},
         xtick={0,1},
         xticklabels={$0$,$\log(\mathit{pdf+1})$},
      }
   }
}
% 
% 
% 
\foreach \p in {1,2,3,4,5,6,7,8,9} {% 
\directlua{tex.print('\string\\message{\p}')}
% 
\begin{tikzpicture}[]
% 
\def\delta{3.3}
\foreach \o[count=\oi from 0] in {0,30,60,90}{
   %
   \pgfmathsetmacro{\px}{2*1.05*\delta*Mod(\oi,2)}
   \pgfmathsetmacro{\py}{1*1.2*\delta*div(\oi,2)}
   % 
   \begin{scope}[shift={(0,-\py)}, local bounding box={BB\oi}]
      \begin{scope}[shift={(\px + 0.00,0)}]
         \begin{polaraxis}[
            PStyle,
         ]
            \addplot [matrix plot*,point meta=explicit,]
               table[meta expr={ln(\thisrowno{2}+1)}]  {%
               \datapath/gt_hists_p_0.\p_o_\o.0_r_0.5_f0_0.0_f1_0.0.dat};
            \coordinate (A\oi) at (0,0);
         \end{polaraxis}
      \end{scope}
      %
      % simulation
      \begin{scope}[shift={(\px + \delta,0)}]
         \begin{polaraxis}[
            PStyle,
         ]
            \addplot [matrix plot*,point meta=explicit,]
               table[meta expr={ln(\thisrowno{2}+1)}]  {%
               \datapath/sim_hists_p_0.\p_o_\o.0_r_0.5_f0_0.0_f1_0.0.dat};
            \coordinate (B\oi) at (0,0);
         \end{polaraxis}
      \end{scope}
   \end{scope}
   % 
   \node[anchor=south, yshift=1.5cm] at ($(A\oi)!0.5!(B\oi)$)
      {\scriptsize $\modelOmega = \SI{\o}{\degree}$};
   \node[anchor=south, yshift=1.5cm] at ($(A\oi)$)
      {\scriptsize model};
   \node[anchor=south, yshift=1.5cm] at ($(B\oi)$)
      {\scriptsize simulation};
   \coordinate (NN\oi) at (BB\oi.north);
   \coordinate (SS\oi) at (BB\oi.south);
}
\coordinate (T) at (current bounding box.north);
\coordinate (B) at (current bounding box.south);
\coordinate (C1) at ($(B0)!0.5!(A1)$);
\coordinate (L) at (current bounding box.west);
\coordinate (R) at (current bounding box.east);
\coordinate (C2) at ($(NN0)!0.5!(SS2)$);
% 
\draw[dashed] (T -| C1) -- (B -| C1);
\draw[dashed] (L |- C2) -- (R |- C2);
\path[] ($(A0)!-0.185!(B1)$) -- ($(A0)!1.185!(B1)$);
% 
\node[anchor=south] at (current bounding box.north) {\small crossing fiber pop., $\modelPsi = \SI{\p0}{\percent}$};
%
\begin{scope}[shift={(current bounding box.south)}, xshift=-5cm, yshift=0em]
\begin{axis}[CBStyle]
\end{axis}
\end{scope}
% 
\end{tikzpicture}
} 
\end{document}
