...
\foreach \x[count=\xi from 0] in {0,30,60,90}{
   %
   \pgfmathsetmacro{\px}{2*1.05*\delta*Mod(\xi,2)}
   \pgfmathsetmacro{\py}{1*1.2*\delta*div(\xi,2)}
   % 
   ...
   \end{scope}
   % 
   \node[anchor=center, yshift=1.5cm] at ($(A\xi)!0.5!(B\xi)$)
      {\scriptsize $\modelInc = \SI{\x}{\degree}$};
   \node[anchor=south, yshift=1.5cm] at ($(A\xi)$)
      {\scriptsize model};
   \node[anchor=south, yshift=1.5cm] at ($(B\xi)$)
...
\foreach \x[count=\xi from 0] in {0,5,10,15,20,25,30,35,40,45}{
   ...
   \pgfmathparse{1*1.2*\delta*div(\xi,2)} \pgfmathsetmacro{\py}{\pgfmathresult}
   % 
   \begin{scope}[shift={(0,-\py)}, local bounding box={BB\xi}]
      \begin{scope}[shift={(\px + 0.00,0)}]
         \begin{polaraxis}[
