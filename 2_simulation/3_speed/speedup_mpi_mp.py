...
if MPI.COMM_WORLD.Get_rank() == 0:
    if MPI.COMM_WORLD.Get_size() == 1 and args.num_proc == 1:
        with open(f'generate_tissue_mpi_mp_v_{simpli.voxel_size}.dat',
                  'w') as f:
            f.write(
                f'{simpli.voxel_size}, {MPI.COMM_WORLD.Get_size()}, {args.num_proc}, {t1 - t0}\n'
            )
    else:
        with open(f'generate_tissue_mpi_mp_v_{simpli.voxel_size}.dat',
                  'a') as f:
...
if MPI.COMM_WORLD.Get_rank() == 0:
    if MPI.COMM_WORLD.Get_size() == 1 and args.num_proc == 1:
        with open(f'simulation_mpi_mp_v_{simpli.voxel_size}.dat', 'w') as f:
            f.write(
                f'{simpli.voxel_size}, {MPI.COMM_WORLD.Get_size()}, {args.num_proc}'
            )
            for t in tt:
                f.write(f', {t}')
            f.write('\n')
    else:
        with open(f'simulation_mpi_mp_v_{simpli.voxel_size}.dat', 'a') as f:
            f.write(
                f'{simpli.voxel_size}, {MPI.COMM_WORLD.Get_size()}, {args.num_proc}'
            )
            for t in tt:
