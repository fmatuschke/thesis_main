""" original from https://github.com/PhTrempe/sknni
    https://arxiv.org/abs/1910.00704

    - found an error in __polar_to_cartesian as issue #1 also discovered.
    - float64
    - np.mean(longitudinal) should be repleaced by circmean, 
      however the results look then wrong
"""
...
class SkNNI:
    ...
    def __init__(self, observations):
        ...
        observations = np.array(observations, dtype=np.float64, copy=False)
        ...
        if (observations[:, 0] <
                MIN_LAT).any() or (observations[:, 0] > MAX_LAT).any() or (
                    observations[:, 1] < MIN_LNG).any() or (observations[:, 1] >
                                                            MAX_LNG).any():
            raise ValueError('Parameter "observations" values out of range')
        ...
        x, y, z = self.__polar_to_cartesian(self.obs_lats_rad,
                                            self.obs_lngs_rad)
    ...
    def __call__(self, interp_coords, k=20, interp_fn=None):
        ...
        interp_coords = np.array(interp_coords, dtype=np.float64, copy=False)
        ...
        if (interp_coords[:, 0] <
                MIN_LAT).any() or (interp_coords[:, 0] > MAX_LAT).any() or (
                    interp_coords[:, 1] < MIN_LNG).any() or (interp_coords[:, 1]
                                                             > MAX_LNG).any():
            raise ValueError('Parameter "interp_coords" values out of range')
        ...
        # Converts polar coordinates to cartesian coordinates
        x, y, z = self.__polar_to_cartesian(interp_lats_rad, interp_lngs_rad)
        ...
        interp_values = interp_fn(knn_lats, knn_lngs, knn_values, p_lats,
                                  p_lngs, k)

        return np.stack((interp_lats_deg, interp_lngs_deg, interp_values),
                        axis=-1)

    @staticmethod
    def __polar_to_cartesian(lat, lng):
        """
        ...
        y = np.sin(lat) * np.sin(lng)
        z = np.cos(lat)
        return x, y, z

    @staticmethod
    def __great_circle_distance(a_lat, a_lng, b_lat, b_lng):
        """
        ...
        # return 2 * np.arcsin(np.sqrt(a)) # same, but could be cheaper to calc
    ...
    def __nddnisd_interp_fn(knn_lats, knn_lngs, knn_values, interp_lats,
        ...
        gc_distance_p_knn = SkNNI.__great_circle_distance(
            interp_lats, interp_lngs, knn_lats, knn_lngs)
        ...
        centroids_lng = np.mean(knn_lngs, axis=0)  # TODO why not circmean?
        gc_distance_centroid_knn = SkNNI.__great_circle_distance(
            centroids_lat, centroids_lng, knn_lats, knn_lngs)
        ...
        ndd_distance_weights /= ndd_distance_weights.sum(axis=-1, keepdims=True)
