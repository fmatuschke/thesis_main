...
def run(parameters):
    ...
    size, radius, mean_length_f, min_radius_f, (psi, omega), n = parameters
...
def check_file(p):
    size, radius, mean_length_f, min_radius_f, (psi, omega), n = p
    file_pref = output_name + f'_psi_{psi:.2f}_omega_{omega:.2f}_r_' \
                            f'{radius:.2f}_v0_{size:.0f}_fl_{mean_length_f:.2f}_' \
                            f'fr_{min_radius_f:.2f}_n_{n}_'
...
def main():
    # Fiber Model
    N_REPEAT = range(24)
    SIZE = [20, 40, 60, 80, 100]  # <- for pre stats ok, no simulation required
    FIBER_RADII = [1.0]
    OBJ_MEAN_LENGTH_F = [2.0]
    OBJ_MIN_RADIUS_F = [2.0]
    PARAMETER = []
    ...
    with mp.Pool(args.num_proc) as pool:
        parameters = list(
            itertools.product(SIZE, FIBER_RADII, OBJ_MEAN_LENGTH_F,
                              OBJ_MIN_RADIUS_F, PARAMETER, N_REPEAT))

        for _ in tqdm.tqdm(
                pool.imap_unordered(run, parameters),
