...
def run(df):
    ...
    return pd.DataFrame([[
        df.f1_theta, df.f1_phi, df.psi, df.radius, phi_x, phi_h, incl_x, incl_h,
        phi_init_x, phi_init_h, incl_init_x, incl_init_h, h, x, y
    ]],
                        columns=[
                            "f1_theta", "f1_phi", "psi", "radius", "phi_x",
                            "phi_h", "incl_x", "incl_h", "phi_init_x",
                            "phi_init_h", "incl_init_x", "incl_init_h",
                            "hist_2d_h", "hist_2d_x", "hist_2d_y"
                        ])


if __name__ == "__main__":

    if False or not os.path.isfile(os.path.join(args.input, "hist",
                                                "data.pkl")):
        df = pd.read_pickle(os.path.join(args.input, "data.pkl"))
        df = df[df.state != "init"]
        ...
        df.to_pickle(os.path.join(args.input, "hist", "data.pkl"))
    else:
        df = pd.read_pickle(os.path.join(args.input, "hist", "data.pkl"))

    sub = df[['f1_theta', 'f1_phi', 'psi']]
    ...
    for f1_theta, f1_phi, psi in sub.itertuples(index=False):
        ...
        for r in df.radius.unique():
            ...
            df_sub = df.query(
                "radius == @r & f1_theta == @f1_theta & f1_phi == @f1_phi & psi == @psi"
            )
            if len(df_sub) != 1:
                warnings.warn(
                    f"len(df_sub) != 1: {r} {f1_theta} {f1_phi} {psi}")
                continue
