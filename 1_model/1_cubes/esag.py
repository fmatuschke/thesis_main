...
def _calc_inv_cov_matrix(mu, gamma_1, gamma_2):
    ...
    inv_cov = np.eye(
        3
    ) + gamma_1 * first_bracket + gamma_2 * second_bracket + factor * third_bracket
...
def _likelihood(x, inv_cov, mu, mu_squared):
    ...
    l = 1 / (2 * np.pi) * c_1**(-3 / 2) * np.exp(
        0.5 * (alpha * alpha - mu_squared)) * m_2
...
def fit(vectors, return_logp=False, print_summary=False):
    '''
    ...
    mle_result = minimize(_log_likelihood,
                          starting_guesses,
                          args=(vectors),
                          method='L-BFGS-B')
    ...
    if return_logp or print_summary:
...
def plot(esag_params,
         n_grid=50,
         plot_esag_density=True,
         const_face_area=True,
         samples=None,
         color_samples=False,
         show_axis=True):
    '''
    ...
    if samples is not None:
        ...
        samples = np.array(samples, copy=True) * 1.01
        ...
        if color_samples:
            ...
            sample_colors_cb = plt.Normalize(vmin=samples_pdf.min(),
                                             vmax=samples_pdf.max())
