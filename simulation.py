...
parser.add_argument('--output',
                    type=str,
                    required=True,
                    help='output h5 file of simulation')
...
VOXEL_SIZES = list(args.voxel_size)
LENGTH = args.length
THICKNESS = 60
...
if os.path.isfile(args.output):
    print("output already exists:", args.output)
    sys.exit(1)
...
with h5py.File(args.output, 'w-') as h5f:
    ...
    try:
        fiber_bundles = fastpli.io.fiber_bundles.load(args.input, '/')
    except:
        raise IOError("input file not found:", args.input)
